<template>
  <div class="p-6">
    ...
    <div class="mb-4 flex items-center gap-3">
      ...
      <select v-model.number="perPage" @change="fetchItems" class="border p-2">
        ...
        <div class="ml-auto">
          <button class="px-3 py-2 bg-indigo-600 text-white rounded" @click="showCreateTopic = true">Create New Topic</button>
        </div>
    </div>
  ...
  <CreateTopicModal :modelValue="showCreateTopic" :subjects="subjects" @update:modelValue="val => showCreateTopic = val" @created="onTopicCreated" />
</template>
...
onMounted(async () => {
  await fetchItems()
  try {
    const res = await fetch(useRuntimeConfig().public.apiBase + '/api/subjects', { credentials: 'include' })
    if (res.ok) {
      const json = await res.json()
      subjects.value = json.subjects || json.data || json || []
    }
  } catch (e) {
    // ignore
  }
