<template>
        ...
        <div v-show="activeTab === 'details'">
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      ...
                      <select v-model="selectedGradeId" class="mt-1 block w-full pl-3 pr-10 py-2 border-gray-300 rounded-md">
                        <option value="">-- Select Grade --</option>
                        <option v-for="g in grades" :key="g.id" :value="g.id">{{ g.name || g.title || g.label }}</option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <select v-model="selectedSubjectId" class="mt-1 block w-full pl-3 pr-10 py-2 border-gray-300 rounded-md">
                        ...
                        <option v-for="s in subjectsByGrade" :key="s.id" :value="s.id">{{ s.name }}</option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <button type="button" class="px-3 py-2 bg-indigo-600 text-white rounded" @click="openCreateTopic">New</button>
                      </div>
                    </div>
                  </div>
  ...
  <!-- Create Topic Modal -->
  <CreateTopicModal :modelValue="showCreateTopic" :subjects="subjectsByGrade" :defaultSubjectId="selectedSubjectId" @update:modelValue="val => showCreateTopic = val" @created="onTopicCreated" />
</template>
...
async function submit() {
  ...
  try {
    const res = await fetchWithTimeout(config.public.apiBase + '/api/quizzes', { method: 'POST', body: data, credentials: 'include' }, 20000)
    if (res.ok) {
...
const debouncedAutoSave = debounce(async () => {
  ...
  try {
    const res = await fetchWithTimeout(config.public.apiBase + '/api/quizzes', { method: 'POST', body: data, credentials: 'include' }, 10000)
    if (res && res.ok) {
      console.log('Autosave successful')
    } else {
      console.warn('Autosave failed or timed out')
    }
  } catch (e) {
    console.warn('Autosave error or aborted', e)
  } finally {
    isSaving.value = false
  }
}, 5000) // 5 second delay
...
async function saveDraft() {
  ...
  try {
    const res = await fetchWithTimeout(config.public.apiBase + '/api/quizzes', { method: 'POST', body: data, credentials: 'include' }, 15000)
    if (res.ok) {
...
onMounted(async () => {
  ...
  try {
    ...
    if (gRes.ok) { const gjson = await gRes.json(); grades.value = gjson.grades || gjson.data || gjson }
...
onBeforeUnmount(() => {
  ...
  editors.value = {}
  // Abort any in-flight autosave/fetch
  try { if (lastFetchController.value) lastFetchController.value.abort() } catch (e) {}
})
...
const subjectsByGrade = computed(() => {
  ...
  return subjects.value.filter(s => String(s.grade_id || s.grade?.id) === String(selectedGradeId.value))
})

function openCreateTopic() {
  // prefill subject in modal if a subject is selected
  showCreateTopic.value = true
}

function onTopicCreated(created) {
  if (!created) return
  // push to topics and preselect
  topics.value.unshift(created)
  form.value.topic_id = created.id || created.topic_id || ''
  // if the created topic has a subject, ensure selectedSubjectId matches
