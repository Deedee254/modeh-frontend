          ...
          <div class="bg-white rounded-xl shadow-sm p-6 text-center">
            <div class="relative inline-block">
              <img 
                ...
                alt="Third place"
                class="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover mx-auto mb-4 border-4 border-orange-200"
              >
...
const fetchData = async () => {
  try {
    ...
    const [tournamentResponse, leaderboardResponse] = await Promise.all([
      useFetch(config.public.apiBase + `/api/tournaments/${route.params.id}`, { credentials: 'include' }),
      useFetch(config.public.apiBase + `/api/tournaments/${route.params.id}/leaderboard`, { credentials: 'include' })
    ])
