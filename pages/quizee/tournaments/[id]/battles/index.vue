...
const fetchData = async () => {
  try {
    ...
    const [tJson, bJson]: any = await Promise.all([
      $fetch(config.public.apiBase + `/api/tournaments/${route.params.id}`, { credentials: 'include' }),
      $fetch(config.public.apiBase + `/api/tournaments/${route.params.id}/battles`, { credentials: 'include' })
    ])
