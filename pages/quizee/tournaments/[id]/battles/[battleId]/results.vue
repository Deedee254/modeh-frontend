            ...
            <div v-for="(q, idx) in result.questions" :key="q.question_id" class="p-4 border rounded-lg">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                <div class="p-3 border rounded-lg">
                  ...
                  <div class="mt-1 text-sm" :class="(mySide() === 'initiator' ? (q.initiator && q.initiator.correct_flag) : (q.opponent && q.opponent.correct_flag)) ? '' : 'text-red-600'" :style="(mySide() === 'initiator' ? (q.initiator && q.initiator.correct_flag) : (q.opponent && q.opponent.correct_flag)) ? { color: '#891f21' } : {}">{{ (mySide() === 'initiator' ? (q.initiator && q.initiator.correct_flag) : (q.opponent && q.opponent.correct_flag)) ? 'Correct' : 'Incorrect' }}</div>
                </div>
                ...
                <div class="p-3 border rounded-lg">
                  ...
                  <div class="mt-1 text-sm" :class="(mySide() === 'initiator' ? (q.opponent && q.opponent.correct_flag) : (q.initiator && q.initiator.correct_flag)) ? '' : 'text-red-600'" :style="(mySide() === 'initiator' ? (q.opponent && q.opponent.correct_flag) : (q.initiator && q.initiator.correct_flag)) ? { color: '#891f21' } : {}">{{ (mySide() === 'initiator' ? (q.opponent && q.opponent.correct_flag) : (q.initiator && q.initiator.correct_flag)) ? 'Correct' : 'Incorrect' }}</div>
                </div>

                <div class="p-3 border rounded-lg" style="background-color: #f7b932; background-color: rgba(247, 185, 50, 0.1); border-color: rgba(247, 185, 50, 0.3)">
