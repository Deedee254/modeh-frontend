<template>
  ...
  <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-6">
        <QuizCard
          v-for="qitem in normalizedQuizzes"
          :key="qitem.id || qitem.quiz_id"
          :to="`/quizee/quizzes/take/${qitem.id}`"
          :startLink="`/quizee/quizzes/take/${qitem.id}`"
          ...
          :description="qitem.description || qitem.summary || ''"
          :marks="qitem.marks"
          :difficulty="qitem.difficulty"
          :quizMaster="qitem['quiz-master']?.name || qitem.user?.name"
          :palette="pickPaletteClass(qitem.topic_id || qitem.topic?.id || qitem.id)"
          :quiz-id="qitem.id"
          :liked="qitem.liked"
          :likes="qitem.likes_count ?? qitem.likes ?? 0"
