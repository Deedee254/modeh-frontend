...
// runtime-load the illustration to avoid Vite trying to statically resolve a missing file
const imgUrl = ref(null)
const imgError = ref(false)

  // Use bundled fallback illustration to avoid runtime HEAD check failures
  if (process.client) {
    imgUrl.value = '/assets/daily-challenge-illustration.svg'
  }
...
let recentBadges = []
  try {
    const { data: attemptsData } = await useFetch(config.public.apiBase + '/quiz-attempts?per_page=5', { credentials: 'include' })
