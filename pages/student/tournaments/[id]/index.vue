...
const fetchTournament = async () => {
  try {
    loading.value = true
    // Use fetch with credentials so cookie-based auth works in the browser
    const resp = await fetch(`/api/tournaments/${route.params.id}`, { credentials: 'include' })
...
const checkRegistrationStatus = async () => {
  try {
    const resp = await fetch(`/api/tournaments/${route.params.id}/registration-status`, { credentials: 'include' })
    const json = await resp.json()
...
const fetchLeaderboard = async () => {
  try {
    const resp = await fetch(`/api/tournaments/${route.params.id}/leaderboard`, { credentials: 'include' })
    const json = await resp.json()
...
const registerForTournament = async () => {
  try {
    ...
    const resp = await fetch(`/api/tournaments/${route.params.id}/join`, { method: 'POST', credentials: 'include' })
    const json = await resp.json()
    if (!resp.ok) {
