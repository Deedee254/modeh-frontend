...
const fetchBattle = async () => {
  try {
    loading.value = true
    const response = await useFetch(`/api/tournaments/${route.params.id}/battles/${route.params.battleId}`)
...
const selectAnswer = async (optionId: string | number) => {
  ...
  try {
    const response = await useFetch(
      `/api/tournaments/${route.params.id}/battles/${route.params.battleId}/questions/${currentQuestion.value.id}/answer`,
      {
        method: 'POST',
...
const submitBattle = async () => {
  try {
    loading.value = true
    await useFetch(
      `/api/tournaments/${route.params.id}/battles/${route.params.battleId}/submit`,
      { method: 'POST' }
    )
...
function startPollingForOpponent() {
  ...
  pollTimer = setInterval(async () => {
    try {
      const resp = await fetch(`/api/tournaments/${route.params.id}/battles/${route.params.battleId}`, { credentials: 'include' })
