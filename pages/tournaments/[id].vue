      ...
      <div class="bg-white p-6 rounded-lg shadow relative">
        ...
        <div class="flex items-center gap-4">
          <template v-if="isLoggedIn">
            <button @click="goToRolePage" class="px-4 py-2 bg-primary text-white rounded">Open in your dashboard</button>
          </template>
          <template v-else>
            <NuxtLink :to="`/register?next=/quizee/tournaments/${tournament.id}`" class="px-4 py-2 bg-primary text-white rounded">Register / Login to participate</NuxtLink>
          </template>
