<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-lg shadow-lg w-full max-w-md p-4">
      ...
      <div class="space-y-3">
        <input v-model="localEmail" ref="dmInput" placeholder="Email or 'support'" class="w-full border px-3 py-2 rounded" @keydown.enter.prevent="start" />
        <div>
...
function start() { emit('start-dm', localEmail.value && localEmail.value.trim()) }

// autofocus the input when modal opens
watch(() => props.show, (v) => { if (v) { setTimeout(() => { try { dmInput.value && dmInput.value.focus && dmInput.value.focus() } catch (e) {} }, 0) } })
</script>
