<template>
  <div v-if="visible" class="relative">
    ...
    <!-- Anchor the missing list to this component to avoid being obscured by sidebars -->
    <div v-if="showMissing && missingList.length" class="absolute right-0 mt-2 z-50 w-64 bg-white border rounded shadow p-3 text-sm">
...
async function load() {
  try {
    ...
    // Always attempt a fresh user fetch on mount to ensure missing_profile_messages
    // and onboarding are up-to-date. This helps avoid stale client-cached user data
    // leaving the progress indicator out-of-sync (especially after recent edits).
    try { await auth.fetchUser(true).catch(() => {}) } catch (e) { }

    const u = auth.user
    if (!u) return

    const onboarding = u?.onboarding || null
    const missing = u?.missing_profile_messages || []
    if (Array.isArray(missing)) missingList.value = missing

    // Determine percent prefering explicit onboarding payload when present
