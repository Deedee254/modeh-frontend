<template>
  <div class="relative w-full h-[300px] sm:h-[450px] lg:h-[600px] overflow-hidden rounded-3xl bg-slate-900/10 backdrop-blur-sm border border-white/5 mx-auto">
    ...
    <div v-if="props.loading || !displayQuizzes.length" class="absolute inset-0 flex items-center justify-center z-20">
      <div class="flex flex-col items-center gap-3 p-6 rounded-2xl bg-slate-900/40 backdrop-blur-md">
        <svg class="h-8 w-8 animate-spin text-brand-500" viewBox="0 0 24 24" fill="none">
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
          <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
        </svg>
        <p class="text-slate-300 text-sm font-medium">Discovering Quizzes...</p>
      </div>
    </div>
    ...
    >
      ...
      >
        <QuizCard
          ...
          class="!shadow-none !border-white/10 !bg-white/5 backdrop-blur-sm hover:!bg-white/10 transition-colors"
        />
      </div>
    </div>
  </div>
