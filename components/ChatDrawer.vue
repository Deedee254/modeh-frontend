<template>
  <div>
  <div v-if="isOpen" class="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm" @click="close"></div>
    ...
    >
      <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Recent Messages</h2>
        <div class="flex items-center gap-2">
          <button @click="goToChat" class="inline-flex items-center duration-300 justify-center gap-2 whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 h-9 rounded-md px-3">
            View All
          </button>
          <button @click="close" class="inline-flex items-center duration-300 justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 hover:bg-slate-100 dark:hover:bg-slate-800 size-10 text-slate-600 dark:text-slate-400" aria-label="Close messages">
      ...
      <div class="h-[calc(100vh-64px)] overflow-y-auto bg-white dark:bg-slate-900 p-2">
        ...
        <!-- Empty State -->
        <div v-else-if="!chats.length" class="p-4 text-center text-slate-500 dark:text-slate-400">
          <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-message-circle h-8 w-8 text-slate-400 dark:text-slate-600"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"></path></svg>
          </div>
          <p>No messages yet</p>
        </div>
        ...
        <div v-else>
          <div
            ...
            class="flex items-start gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg cursor-pointer transition-colors"
          >
            <div class="relative">
              <span class="size-10">
            ...
            <div class="flex-1 space-y-1 overflow-hidden">
              <div class="flex items-center justify-between">
                <p class="font-medium text-slate-900 dark:text-white" :class="((chat.unread_count ?? chat.unread) || 0) > 0 ? 'font-semibold' : ''">{{ chat.name }}</p>
                <span class="text-xs text-slate-500 dark:text-slate-400">{{ chat.time }}</span>
              </div>
              <p class="text-sm truncate text-slate-600 dark:text-slate-300">{{ chat.message }}</p>
            </div>
          </div>
        </div>
      </div>
