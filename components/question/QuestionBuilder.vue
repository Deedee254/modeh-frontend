...
defineExpose({ openQuestionModal, openQuestionBankModal })
      ...
      <div v-if="requiresOptions && form.type !== 'fill_blank'" class="mt-3">
        ...
        <div class="space-y-2 mt-2">
                  <div v-for="(opt, idx) in form.options" :key="opt.id" class="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
                    <input :data-qb-option-input="opt.id" v-model="opt.text" placeholder="Option text" class="w-full sm:flex-1 rounded px-3 py-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-sm text-gray-800 dark:text-gray-200" />
            <div class="flex items-center gap-2">
              <template v-if="form.type === 'multi'">
                <input type="checkbox" :checked="(form.answers || []).includes(opt.text)" @change="() => toggleAnswer(idx)" title="Mark correct" />
              </template>
              <template v-else>
                <input type="radio" name="single-correct" :checked="(form.answers || [])[0] === opt.text" @change="() => setSingleAnswer(idx)" title="Mark correct" />
              </template>
    ...
    <aside class="lg:col-span-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 text-sm text-gray-600 dark:text-gray-300">
      ...
      <div v-if="requiresOptions && form.type !== 'fill_blank'" class="mb-3">
        <label class="text-xs font-semibold text-gray-500 dark:text-gray-300">Options</label>
        <div class="space-y-2 mt-2">
          <div v-for="(opt, i) in form.options" :key="opt.id" class="rounded-md bg-white dark:bg-gray-900/30 p-2 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-2"><span class="font-semibold text-xs text-gray-500 dark:text-gray-400">{{ i + 1 }}</span><div class="text-sm" v-text="opt.text || ('Option ' + (i+1))"></div></div>
          </div>
        </div>
      </div>
...
// simple incremental id generator for option keys (avoids UUIDs/randomness)
let nextOptionId = 1

// Represent options as objects { id, text } internally so keys are stable and
// DOM updates / focus behave reliably. We convert back to plain string array
// when building the payload for the server.
const getInitialFormState = () => {
  const rawOptions = props.prefill?.options ? [...props.prefill.options] : ['', '']
  const initialOptions = rawOptions.map(o => ({ id: nextOptionId++, text: o }))
  const initialAnswers = (() => {
    const a = props.prefill?.answers ? [...props.prefill.answers] : []
    if (Array.isArray(a) && a.length && a.every(x => typeof x === 'number')) {
      return a.map(i => (initialOptions[i] && initialOptions[i].text) || '').filter(v => v !== undefined && v !== null)
    }
    return a
  })()

  return {
    body: props.prefill?.body || 'Enter your text here…',
...
function setSingleAnswer(i) {
  const optObj = form.value.options?.[i]
  form.value.answers = optObj === undefined ? [] : [optObj.text]
}

function addOption() {
  // Use immutable update to ensure reactivity and then focus the new input.
  // Await two ticks and provide a short timeout fallback to handle edge cases
  // where the DOM update may not be immediate (transitions, client-only components).
  // Add option object with uid and empty text.
  // Use .push() on the existing array to avoid replacing the array reference
  // which can be problematic in some older Vue setups.
  const newOpt = { id: nextOptionId++, text: '' }
  try { console.debug('[QuestionBuilder] adding option', newOpt, 'before', form.value.options?.map(o=>o.text)) } catch(e) {}
  ;(form.value.options || (form.value.options = [])).push(newOpt)
  try { console.debug('[QuestionBuilder] options after add', form.value.options.map(o=>o.text)) } catch(e) {}
  nextTick(() => {
    // second tick to be extra-safe
    nextTick(() => {
      try {
        // try to focus the newly added input by its data attribute id
        const selector = `[data-qb-option-input="${newOpt.id}"]`
        const el = document.querySelector(selector)
        if (el && typeof (el as any).focus === 'function') {
          ;(el as any).focus()
          try { const len = ((el as any).value || '').length; (el as any).setSelectionRange && (el as any).setSelectionRange(len, len) } catch (e) {}
          return
        }
      } catch (e) {}
      // fallback: small timeout and focus last input
      setTimeout(() => {
        try {
          const inputs = document.querySelectorAll('[data-qb-option-input]')
          const last = inputs[inputs.length - 1]
          if (last && typeof (last as any).focus === 'function') (last as any).focus()
        } catch (e) {}
      }, 50)
    })
  })
...
function removeOption(i) {
  if (form.value.options.length > 1) {
    ...
    if (Array.isArray(form.value.answers) && removed !== undefined) {
      // remove any answers matching the removed option text
      form.value.answers = form.value.answers.filter(a => a !== removed.text)
    }
  }
}
    ...
    for (const key in payload) {
      ...
      if (value === null || value === undefined) continue
          ...
          if (Array.isArray(value)) {
            // If this is the options array of objects, convert to plain text array
            if (key === 'options' && value.length && typeof value[0] === 'object' && 'text' in value[0]) {
...
function onReset() {
  form.value = {
    body: '<p></p>',
    type: 'mcq',
    options: [{ id: nextOptionId++, text: '' }, { id: nextOptionId++, text: '' }],
    answers: [],
    difficulty: 2,
  ...
  if (editorRef.value?.setHTML) {
    try { editorRef.value.setHTML('<p></p>') } catch (e) {}
  } else if (editorRef.value?.setContent) {
    // backward-compat: some editor refs may expose setContent
    try { editorRef.value.setContent('<p></p>') } catch (e) {}
  }
}
